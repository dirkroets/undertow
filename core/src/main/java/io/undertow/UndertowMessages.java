...
//    IllegalArgumentException maxSizeMustBeLargerThanOne();
    ...
    @Message(id = 211, value = "Wild cards are only supported at the end of a template path")
    IllegalArgumentException wildCardsOnlyAtEndOfTemplate();

    @Message(id = 212, value = "Illegal character '%s' is contained in the URL path segment '%s' at position %s")
    IllegalArgumentException illegalCharacterInPathSegment(char character, String segment, int position);

    @Message(id = 213, value = "updateDefaultTargetFactory cannot be called after having added templates")
    IllegalStateException defaultTargetUpdatedAfterTemplatesAdded();

    @Message(id = 214, value = "The builder already contains a template with the same pattern for '%s'")
    IllegalArgumentException duplicateUrlPathTemplate(String template);

    @Message(id = 215, value = "The 'defaultTargetFactory' must be set before calling 'build'")
